...
- **正規表現リテラルの追跡(2026-07 強化 / must-fix・再発防止)**: `str.split(/\//)`・`/^\/api\//`・`/[/]/`、および文字クラス内に `//` や `/*` を含む `/[a//z]/`・`/[/*]/` を `//`・`/*` コメント開始と誤認し、その行の残り(`markDone()` 等の正当なコード)を丸ごと削っていた不具合を修正。学習者が提出する任意コードに対し `ignoreComments:true` の source check が実行されるため、正しいコードが不合格になる実害があった。
  - **判定規則(安全側の非対称性が肝)**: 直前の意味トークンが**明確な値**(識別子/数値/文字列/正規表現/`]`)なら `/` は除算。それ以外(演算子・`(`・`,`・`=`・`return`/`default`/`case` 等のキーワード直後、および**曖昧な `}`・`)`**)は正規表現の開始とみなして走査(`scanJsRegex`。文字クラス `[...]` と `\` エスケープを尊重)し、失敗(改行/未終端/EOF)したら除算にフォールバックする。
...
- 既知の制限(いずれも「コメントを消し残す」安全側の失敗で、コードは決して削らない): (1) 曖昧位置(`}`・`)`・キーワード)直後に本物の除算があり同一行に `//`・`/* */` が続く場合(`f() / 2; /* c */ x`)、正規表現とみなした走査がそのコメントを取り込み消し残すことがある(改行を挟めば `scanJsRegex` が null を返し除算に戻ってコメントは除去される。明確な値の直後は曖昧でないためこの限りでない)。(2) インライン `<script>` / `<style>` 内の JS/CSS コメントは除去対象外(HTML コメントのみ)。いずれも誤除去で solution が落ちることはなく、仮に穴が生じてもステージ2が検出する
