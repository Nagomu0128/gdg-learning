...
- 既知の制限(いずれも「コメントを消し残す」安全側の失敗で、コードは決して削らない): (1) **意図的な割り切り** — `{}/re/` や `if(x)/re/` のコード削除を防ぐため `}`・`)` 直後は安全側として regex に倒す。その副作用で、曖昧位置(`}`・`)`・キーワード)直後に**本物の除算**があり同一行に `//`・`/* */` が続く場合(`f() / 2; /* c */ x`)、正規表現とみなした走査がそのコメントを取り込み消し残すことがある(`ignoreComments` は元来 best-effort で、コメントが多少残っても pattern 一致には通常無害 — 安全側への劣化)。改行を挟めば `scanJsRegex` が null を返し除算に戻ってコメントは除去される。明確な値の直後は曖昧でないためこの限りでない。(2) インライン `<script>` / `<style>` 内の JS/CSS コメントは除去対象外(HTML コメントのみ)。いずれも誤除去で solution が落ちることはなく、仮に穴が生じてもステージ2が検出する
