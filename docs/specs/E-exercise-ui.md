...
判定メッセージ aria-live、タブは role=tablist、モーダルフォーカストラップ、全操作キーボード可。エディタの Tab は行インデント・Escape 直後の Tab はフォーカス移動(ADR #20)。この脱出手段はエディタ直下の常時ヒント + aria-describedby で周知する。
